    ...
    function Airport() {
        ...
        this.getData = function () {
            ...
            for (var i = 0; i < this.listOfFlights.length; i++) {
                res += this.listOfFlights[i].date + ',' + this.listOfFlights[i].relation + '\n';
                for (var j = 0; j < this.listOfFlights[i].listOfPass.length; j++) {
                    res += this.listOfFlights[i].listOfPass[j].getData();
                }
                res += '\n';
            }
            return 'Airport : ' + this.name + ', total passengers : ' + (this.listOfFlights[0].listOfPass.length + this.listOfFlights[1].listOfPass.length) + '\n'
                + res;
